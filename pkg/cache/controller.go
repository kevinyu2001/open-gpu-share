...
func NewController(clientset *kubernetes.Clientset, kubeInformerFactory kubeinformers.SharedInformerFactory, stopCh <-chan struct{}) (*Controller, error) {
	...
	ConfigMapInformerSynced = cmInformer.Informer().HasSynced

	// Start informer goroutines.
	go kubeInformerFactory.Start(stopCh)

	// Create scheduler Cache
	c.schedulerCache = NewSchedulerCache(c.nodeLister, c.podLister)
	...
	if ok := clientgocache.WaitForCacheSync(stopCh, ConfigMapInformerSynced); !ok {
...
func (c *Controller) GetSchedulerCache() *SchedulerCache {
